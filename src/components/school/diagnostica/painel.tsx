...
/**
 * Painel de evolução diagnóstica, usado em dois lugares:
 *
 * - no painel de gestão, completo;
 * - na página pública, com `publico`, que esconde as abas de aluno.
 *
 * O corte é proposital: resultado de turma é informação da escola e pode
 * circular; nome de criança ligado à habilidade que ela não domina é dado
 * sensível (LGPD, art. 11) e fica só para quem tem login.
 */
export function PainelEvolucao({ publico = false }: { publico?: boolean }) {
  ...
  useEffect(() => {
    supabase
      ...
      .select("dados")
      .eq("id", publico ? "2026-II-publico" : "2026-II")
      .maybeSingle()
      .then(({ data, error }) => {
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3 print:hidden">
        <div>
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            I e II Avaliação Diagnóstica 2026 (SEME) lado a lado: o que avançou e onde a escola
            ainda trava.
            {!publico && " Nas abas Alunos e Intervenção, quem precisa de apoio agora."}
          </p>
        </div>
